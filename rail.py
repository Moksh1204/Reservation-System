...
class price_changes():
    def price_declare(self):
        ...
        self.price = {'first a/c class' : 3000, 'second a/c class' : 2000, 'third a/c class' : 1200, 'sleeper' : 800}

class seats_changes():
    def seats_declare(self):
        # creating a dictionary with compartment names as keys and its number of total seats as values.
        self.seats = {'first a/c class' : 50, 'second a/c class' : 100, 'third a/c class' : 150, 'sleeper' : 200}

class refund_percent_changes():
    def refund_percent_declare(self):
        self.confirm_refund_percent = 30
        self.waiting_refund_percent = 50
...
class reservation():
    ...
    def input(self):
        # taking passenger's information from user
    ...
    def choice(self):
        ...
        option = int(input("Choose your option from the following:\n1. book\n2. cancel\n=> Your option:- "))
...
# creating child class book for booking purpose
class book(reservation, price_changes, seats_changes, refund_percent_changes):
    ...
    def compartment(self):
        self.compartment = int(input("Enter the compartment name that you want :-\n 1. first a/c class. \n 2. second a/c class.\n 3. third a/c class\n 4. sleeper\n"))

        if self.compartment == 1:
            self.compartment = "first a/c class"
        elif self.compartment == 2:
            self.compartment = "second a/c class"
        elif self.compartment == 3:
            self.compartment = "third a/c class"
        elif self.compartment == 4:
            self.compartment = "sleeper"
        else:
    ...
    def availablity(self):
        ...
        if self.seats[self.compartment] > 0:
            ...
            self.coupon = 'c' + self.fname[0] + self.lname[0] + '-' + self.compartment
            ...
            self.coupon = 'w' + self.fname[0] + self.lname[0] + '-' + self.compartment
    ...
    def payment(self):
        ...
        self.mode = input("Payment Mode : Cash or Card or Net-banking?")
    ...
    def database(self):
        with open('bookings.csv', 'a', newline='') as file:
            ...
            writer.writerow(
                [self.coupon, self.fname, self.lname, self.age, self.gender, self.compartment, self.availability,
                 self.price[self.compartment], self.mode, self.completion])
        file.close()

    # printing of the full ticket after completion of the booking process
    def booked_ticket_printing(self):
        print()
        print("DEMAND FOR BOOKING A TICKET");
...
class cancel(reservation, price_changes, seats_changes, refund_percent_changes):
    ...
    def refund(self):
        if self.coupon[0] == 'c':
            ...
            self.re = self.price[self.coupon[4:]] * (self.confirm_refund_percent / 100);
    ...
    def cancelled_ticket_printing(self):
        ...
        print("Your Coupon Code:", self.coupon)
        print("Your refund is", self.re, "rupees.");
        print("You saved (", self.price[self.coupon[4:]], "-", self.re, ") =", self.price[self.coupon[4:]] - self.re,
              "rupees.")
        print("Process completed.");
        print()
...
while (True):
    ...
    if int(input("\nEnter 1 to continue with railway reservations, 0 to stop: ")) != 1:
        print("Thanks for choosing our website.See you again.")
        break;
